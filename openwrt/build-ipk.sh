#!/usr/bin/env bash
# =============================================================================
# build-ipk.sh — 生成单个「all 架构」的 luci-app-cfstmgrd OpenWrt .ipk
#
#   该包不含 CPU 二进制，只装壳子（procd 脚本 + UCI 配置 + cfstmgrd-fetch + LuCI）。
#   一个包到处可装，安装/网页操作时由 cfstmgrd-fetch 按本机 CPU 联网下载
#   对应版本的二进制到 /usr/bin/cfstmgrd。
#
# 参数：
#   --version <x.y.z>   版本号（也可用环境变量 VERSION）。这是 cfstmgrd-fetch
...
# 随包文件
INITD_SRC="$ROOT_DIR/openwrt/files/etc/init.d/cfstmgrd"
CONFIG_SRC="$ROOT_DIR/openwrt/files/etc/config/cfstmgrd"
FETCH_SRC="$ROOT_DIR/openwrt/files/usr/sbin/cfstmgrd-fetch"
...
# LuCI web 壳子
LUA_CTRL="$ROOT_DIR/openwrt/luci-app-cfstmgrd/luasrc/controller/cfstmgr.lua"
LUA_VIEW="$ROOT_DIR/openwrt/luci-app-cfstmgrd/luasrc/view/cfstmgr/main.htm"
ACL_LUCI="$ROOT_DIR/openwrt/luci-app-cfstmgrd/root/usr/share/rpcd/acl.d/luci-app-cfstmgrd.json"
UCIDEF_LUCI="$ROOT_DIR/openwrt/luci-app-cfstmgrd/root/etc/uci-defaults/40_luci-cfstmgrd"
...
WORK="$(mktemp -d 2>/dev/null || mktemp -d -t cfstmgrd-ipk)"
cleanup() { [ -n "${WORK:-}" ] && rm -rf "$WORK"; }
trap cleanup EXIT INT TERM

# 生成随包 VERSION 文件（cfstmgrd-fetch 默认拉此版本）
...
OUTFILE="$OUT/luci-app-cfstmgrd_${VERSION}-${PKG_RELEASE}_all.ipk"
